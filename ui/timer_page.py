...
class TimerPage(QWidget):
    ...
    def _stats_row(self):
        ...
        for icon, val, title, sub, col, highlight in items:
            ...
            icon_box.setFixedSize(40, 40)
            icon_box.setAlignment(Qt.AlignmentFlag.AlignCenter)
            icon_box.setStyleSheet("font-size: 20px; background: rgba(255,255,255,0.07); border-radius: 10px; margin-bottom: 6px;")
            top.addWidget(icon_box)
            top.addStretch()
            val_lbl = QLabel(str(val))
            val_lbl.setMinimumHeight(32)
            val_lbl.setStyleSheet(f"font-size: 22px; font-weight: bold; color: {TEXT_PRIMARY}; background: transparent; padding-top: 6px;")
            t_lbl = QLabel(title)
